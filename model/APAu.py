...
class APAu(object):
    ...
    def get_loss(self, inputs_source, inputs_target, labels_source, labels_target,
        ...
        outputs_src, outputs_tgt = outputs.chunk(2)
        classifier_loss = nn.CrossEntropyLoss()(outputs_src, labels_source)

        total_loss = 0.
        total_loss += classifier_loss

        if args.vat_loss_weight > 0:
            features_src, features_tgt = features.chunk(2)
            ...
            for i in range(num_iters):
                ...
                logp_hat = F.log_softmax(_outputs, dim=1)
            ...
            r_adv = d * eps
            act = features_tgt + r_adv.detach().clone()

            _outputs = self.c_net_cls(act)
            logp_hat = F.log_softmax(_outputs, dim=1)
        ...
        if args.fixmatch_loss_weight > 0.0:
            ...
            for i in range(target_rand_aug_size):
                ...
                aug_loss += (F.cross_entropy(outputs_tgt_aug, pred_u, reduction='none') *
                              max_prob.ge(args.fixmatch_loss_threshold).float().detach()).mean()
